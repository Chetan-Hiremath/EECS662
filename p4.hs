...
evalDyn e (Plus x y) = do {(Num x') <- (evalDyn e x);
                           (Num y') <- (evalDyn e y);
                           if (x' + y') < 0 then Nothing else return (Num (x' + y'))}
evalDyn e (Minus x y) = do {(Num x') <- (evalDyn e x);
                            (Num y') <- (evalDyn e y);
                            if (x' - y') < 0 then Nothing else return (Num (x' - y'))}
evalDyn e (Mult x y) = do {(Num x') <- (evalDyn e x);
                           (Num y') <- (evalDyn e y);
                           if (x' * y') < 0 then Nothing else return (Num (x' * y'))}
evalDyn e (Div x y) = do {(Num x') <- (evalDyn e x);
                          (Num y') <- (evalDyn e y);
                          if (y' == 0) then Nothing else return (Num (x' `div` y'))}
evalDyn e (Exp x y) = do {(Num x') <- (evalDyn e x);
                          (Num y') <- (evalDyn e y);
                          if (x' ^ y') < 0 then Nothing else return (Num (x' ^ y'))}
evalDyn e (If0 x y z) = do {(Num x') <- (evalDyn e x);
                            if (x' == 0) then (evalDyn e y) else (evalDyn e z)}
evalDyn e (Id x) = (lookup x e)
evalDyn e (Lambda x y) = return (Lambda x y)
evalDyn e (App a b) = do {(Lambda x y) <- (evalDyn e a);
                          b' <- (evalDyn e b);
                          (evalDyn ((x,b'):e) y)}
...
evalStat e (Plus x y) = do {(NumV x') <- (evalStat e x);
                            (NumV y') <- (evalStat e y);
                            if (x' + y') < 0 then Nothing else return (NumV (x' + y'))}
evalStat e (Minus x y) = do {(NumV x') <- (evalStat e x);
                             (NumV y') <- (evalStat e y);
                             if (x' - y') < 0 then Nothing else return (NumV (x' - y'))}
evalStat e (Mult x y) = do {(NumV x') <- (evalStat e x);
                            (NumV y') <- (evalStat e y);
                            if (x' * y') < 0 then Nothing else return (NumV (x' * y'))}
evalStat e (Div x y) = do {(NumV x') <- (evalStat e x);
                           (NumV y') <- (evalStat e y);
                           if (y' == 0) then Nothing else return (NumV (x' `div` y'))}
evalStat e (Exp x y) = do {(NumV x') <- (evalStat e x);
                           (NumV y') <- (evalStat e y);
                           if (x' ^ y') < 0 then Nothing else return (NumV (x' ^ y'))}
evalStat e (If0 x y z) = do {(NumV x') <- (evalStat e x);
                             if (x' == 0) then (evalStat e y) else (evalStat e z)}
evalStat e (Id x) = (lookup x e)
evalStat e (Lambda x y) = return (ClosureV x y e)
evalStat e (App a b) = do {(ClosureV x y z) <- (evalStat e a);
                           b' <- (evalStat e b);
                           (evalStat ((x,b'):z) y)}
...
elabTerm (AppX a b) = (App (elabTerm a) (elabTerm b))
...
evalReader (Plus x y) = do {(NumV x') <- (evalReader x);
                            (NumV y') <- (evalReader y);
                            if (x' + y') < 0 then error "fail" else return (NumV (x' + y'))}
evalReader (Minus x y) = do {(NumV x') <- (evalReader x);
                             (NumV y') <- (evalReader y);
                             if (x' - y') < 0 then error "fail" else return (NumV (x' - y'))}
evalReader (Mult x y) = do {(NumV x') <- (evalReader x);
                            (NumV y') <- (evalReader y);
                            if (x' * y') < 0 then error "fail" else return (NumV (x' * y'))}        
evalReader (Div x y) = do {(NumV x') <- (evalReader x);
                           (NumV y') <- (evalReader y);
                           if (y' == 0) then error "fail" else return (NumV (x' `div` y'))} 
evalReader (Exp x y) = do {(NumV x') <- (evalReader x);
                           (NumV y') <- (evalReader y);
                           if (x' ^ y') < 0 then error "fail" else return (NumV (x' ^ y'))} 
evalReader (If0 x y z) = do {(NumV x') <- (evalReader x);
                             if (x' == 0) then (evalReader y) else (evalReader z)}
evalReader (Id x) = do {e <- ask;
                        case (lookup x e) of
                                 Just a -> return a
                                 Nothing -> error "fail"}
evalReader (Lambda x y) = do {e <- ask;
                              return (ClosureV x y e)}
evalReader (App a b) = do {(ClosureV x y e) <- (evalReader a);
                           b' <- (evalReader b);
                           local (useClosure x b' e) (evalReader y)}
